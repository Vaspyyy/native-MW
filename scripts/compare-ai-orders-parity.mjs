...
function compare(left, right, path = "$") {
	if (typeof left === "number" && typeof right === "number") {
		if (
			...
			return null;
		return `${path}: ${left} != ${right} (float tolerance ${TOLERANCE})`;
	}
	if (
		left === null ||
		right === null ||
...
if (!leftPath || !rightPath) {
	throw new Error(
		"usage: compare-ai-orders-parity.mjs <left-report.json> <right-report.json>",
	);
}
...
if (mismatch) {
	console.error(`AI order parity: mismatch\n${mismatch}`);
	process.exitCode = 1;
} else {
	console.log("AI order parity: ok");
}
